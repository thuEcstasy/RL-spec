...
from soft_flexattn_cllm_trainer_multiblock_v3 import CllmTrainer
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    # Multi-noisy capture schedule for the v3 trainer. Each value is an "accepted
    # token count" inside one Jacobi rollout call at which a noisy snapshot will
    # be captured. Given block_size=N=16, the default "[4,6,8,10,12]" produces
    # m=5 noisy chains per trajectory position, all paired with the same clean
    # block.
    capture_lens: str = field(
        default="4,6,8,10,12",
        metadata={"help": "Comma-separated list of accepted-token targets for multi-noisy capture"},
    )
    ...
    val_block_size: int = field(
        default=0,
        metadata={"help": "Block size (n_token_seq_len) to use for validation Jacobi decoding. 0 = use training max_new_tokens"},
    )
...
def make_online_jacobi_data_module(tokenizer, prompt_data, training_args) -> Dict:
    ...
    # Parse capture_lens CLI string into List[int]; empty string → single-noisy mode.
    raw_lens = (training_args.capture_lens or "").strip()
    if raw_lens:
        parsed_capture_lens = [int(x) for x in raw_lens.split(",") if x.strip()]
    else:
        parsed_capture_lens = None
    ...
    online_cfg = OnlineJacobiConfig(
        ...
        capture_lens=parsed_capture_lens,
    )
