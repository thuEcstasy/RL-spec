...
class CllmTrainer(Trainer):
    ...
    # >>> BSZ>1: NEW helper to get batch size safely
    def _batch_size(self, inputs) -> int:
        ids = inputs["input_ids"]
        return ids.shape[0] if ids.dim() > 1 else 1

    # >>> BSZ>1: UPDATED to support per-sample unpacking via idx
    def _unpack_sample(self, inputs, idx: int = 0):
        """
        Extract *one* sample (supports per_device_train_batch_size >= 1).
        Required keys:
          - input_ids: [B, L] or [L]
          - prompt_ids_len: scalar or [B]
          - traj_position_indices: [B, ... T ...] (we'll flatten to length T)
        """
        ids = inputs["input_ids"]
        if ids.dim() == 1:
            input_ids = ids
        ...
        if isinstance(prompt_len, torch.Tensor):
            # handle 0-dim, 1-dim, or more
            if prompt_len.dim() == 0:
                prompt_len = self._to_int(prompt_len)
            else:
                prompt_len = self._to_int(prompt_len[idx])
        else:
            prompt_len = self._to_int(prompt_len)

        tpi = inputs["traj_position_indices"]
        # robustly grab the sequence of positions for this sample and flatten
        if isinstance(tpi, (list, tuple)):
            tpi_b = tpi[idx]
            traj_position_indices = torch.as_tensor(tpi_b).reshape(-1)
        else:
            tpi = torch.as_tensor(tpi)
            if tpi.dim() == 1:
                traj_position_indices = tpi
            else:
                traj_position_indices = tpi[idx].reshape(-1)
        T = int(traj_position_indices.numel())
...
class CllmTrainer(Trainer):
    ...
    # >>> BSZ>1: FACTORED core per-sample computation (no backward here)
    def _compute_losses_for_one_sample(self, model, input_ids, prompt_len, T):
        L = input_ids.size(0)
        ...
        T_soft = getattr(self.args, "distill_temperature", 1.0)
        drop_last_offset = False
        offs = torch.arange(self.max_new_tokens - (1 if drop_last_offset else 0), device=self.args.device)
        ...
        for j in range(T):
            ...
            ks, ls = k_starts[j], l_starts[j]
            pair_keep = self._block_keep_mask_divergence_and_eos(
                input_ids, ks, ls, self.max_new_tokens, eos_id=eos_id, drop_last_offset=drop_last_offset
            )
            if pair_keep.any():
                sp = ks + offs[pair_keep]
                tp = ls + offs[pair_keep]
        ...
        return total_loss, loss_ar, loss_consistency

    def _one_pass_losses_step(self, model, inputs):
        # >>> BSZ>1: NEW micro-batch loop
        ...
        for b in range(B):
            input_ids, prompt_len, T = self._unpack_sample(inputs, idx=b)
            total_loss_b, loss_ar_b, loss_consistency_b = self._compute_losses_for_one_sample(
                model, input_ids, prompt_len, T
            )

            # Optional freeing of cached blocks each microstep (kept per original code spirit)
            torch.cuda.empty_cache()

            # Scale by 1/B to get the mean loss gradient when summed over micro-batches
            scaled = total_loss_b / B
            ...
            # accumulate for logging / return
            total_loss_sum = total_loss_sum + total_loss_b.detach()
            ar_loss_sum    = ar_loss_sum + loss_ar_b.detach()
            cons_loss_sum  = cons_loss_sum + loss_consistency_b.detach()

        if torch.distributed.is_available() and torch.distributed.is_initialized():
            torch.distributed.barrier()

        avg_total = total_loss_sum / B
        avg_ar = ar_loss_sum / B
        avg_cons = cons_loss_sum / B

        if self.args.local_rank == 0:
            wandb.log({
                "ar loss": float(avg_ar.cpu()),
                "consistency loss": float(avg_cons.cpu())
            })
