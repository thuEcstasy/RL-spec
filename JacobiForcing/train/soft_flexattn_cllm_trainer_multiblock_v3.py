...
class CllmTrainer(Trainer):
    ...
    #   - k_j^i (noisy):    prompt + k_{j'<j}^i (SAME chain i only) + own k_j^i block (causal)
    ...
    def _build_block_mask(self, L: int, prompt_len: int, T: int, heads: int):
        ...
        def mask_mod(b, h, q, k):
            ...
            #   - own block causal                 (same group, same chain, k <= q)
            noisy_prev_chain_k = is_noisy_k & same_chain & (group_k < group_q)
            noisy_self_k = is_noisy_k & same_group & same_chain & (k <= q)
