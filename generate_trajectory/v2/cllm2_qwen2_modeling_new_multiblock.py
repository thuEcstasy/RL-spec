...
def diffusion_forward(
    ...
    multiblock_size: Optional[int] = 4,
    multiblock_draft_futures: Optional[List[torch.Tensor]] = None,
    ):
    ...
    if prefill_phase: # prefill phase, just compute the keys & values of prompt
        ...
        ### if multiblock_draft_futures is available --> reuse
        ### if not available --> initialize with random tokens
        for _ in range(multiblock_size):
            if multiblock_draft_futures and len(multiblock_draft_futures) > 0:
                q_sampled = multiblock_draft_futures.pop(0)
            else:
        ...
        # TODO: FIX THE FOLLOWING CODE TO ADAPT MULTI-BLOCK DECODING
        # total accepted token count by batch size
        total_accepted = torch.zeros(batch, dtype=torch.long, device=input_ids.device)
        unfinished_mask = total_accepted < n_token_seq_len
        itr = 0
        q_sampled_ids = {}
        delete_key_values_length = n_token_seq_len * multiblock_size + 1
        ...
        while True:
            ...
            ### 2）Verify and speculate within a forward pass
            ...
            if not len(finished_n_poss)==0:
                ...
                # calculate offset
                # offset should be the same across batch size
                offset = (n_token_seq_len * multiblock_size - total_accepted[orig_idxs]).tolist()[0]
                #print(f"offset: {offset}")

                if offset > 0:
                    for layer_idx in range(len(mid_point.key_cache)):
                        #print(f"mid point key cache size: {mid_point.key_cache[layer_idx].shape}")
                        #print(f"selected mid point key cache size: {mid_point.key_cache[layer_idx][..., :-offset, :].shape}")
                        mid_point.key_cache[layer_idx] = mid_point.key_cache[layer_idx][..., :-offset, :].contiguous()
                        mid_point.value_cache[layer_idx] = mid_point.value_cache[layer_idx][..., :-offset, :].contiguous()

                # merge finished past key values
        ...
        # cache multiblock_draft_futures
        multiblock_draft_futures = out[..., -((multiblock_size-1) * n_token_seq_len):]
        # trim output
        out = out[..., :-(n_token_seq_len * (multiblock_size-1))]

        return out, past_key_values, multiblock_draft_futures

# TODO: SUPPORT MULTI-BLOCK DECODING
@torch.inference_mode()
    ...
    if prefill_phase: # prefill phase, just compute the keys & values of prompt
        ...
        while True:
            ...
            for n, idx in enumerate(idx_unfin):
                ...
                # PAD enforcement
                draft_start = pad_lens[idx] + prompt_lens[idx]
                fill_start  = draft_start + total_accepted[idx]
                fill_end    = draft_start + n_token_seq_len
                if fill_start < fill_end:
                    out[idx, fill_start:fill_end] = tokenizer.pad_token_id
