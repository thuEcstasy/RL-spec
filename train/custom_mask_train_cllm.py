...
# ### REMOVED: direct DS usage from this script (Accelerate will own DS engine)
# import deepspeed
from transformers.integrations import HfDeepSpeedConfig

from accelerate import Accelerator, DeepSpeedPlugin  # ### ADDED
...
def safe_save_model_for_hf_trainer(model, tokenizer, output_dir, accelerator=None):  # ### CHANGED
    """
    Save consolidated weights. Works with Accelerator/DeepSpeed:
...
def train():
    ...
    config.use_cache = False  # training

    # ---------- Ensure partitioned loading for ZeRO-3/offload ----------
    dschf = HfDeepSpeedConfig(training_args.deepspeed)  # ### ADDED (keep alive)
    ...
    # Gradient checkpointing: enable on model (Accelerate/DS will honor it)
    ...
    # ---------- Optimizer / Scheduler ----------
    # Filter trainable params (important for LoRA/QLoRA)
    params = (p for p in model.parameters() if p.requires_grad)
    # You can swap for your preferred optimizer; keep betas/eps sensible for bf16
    ...
    # ---------- Let Accelerate + DeepSpeed wrap everything ----------
    ...
    trainer = CllmTrainer(
        model=model,                      # <-- already DS/Accelerate-prepared
        tokenizer=tokenizer,
        args=training_args,
        train_dataset=data_module["train_dataset"],
        accelerator=accelerator,          # <-- pass it so your trainer uses accelerator.backward, etc.
        optimizer=optimizer,              # <-- prepared optimizer
        lr_scheduler=lr_scheduler,        # <-- prepared scheduler
        train_dataloader=train_dataloader # <-- prepared dataloader
    )
    ...
    # ---------- Save ----------
    # Prefer saving inside trainer where you can call engine.save_checkpoint if needed.
    # Here we do a safe unwrap and save.
