...
class CllmTrainer(Trainer):
    ...
    def _one_pass_losses_step(self, model, inputs):
        ...
        # ---- FlexAttention: build BlockMask & forward once ----
        ...
        if len(student_positions) == 0:
            ...
            # Hard targets: token ids from the aligned last_j positions
            target_ids = input_ids[teacher_positions].to(device=self.args.device, dtype=torch.long)
            loss_consistency = F.cross_entropy(student_logits_sel, target_ids, reduction="mean")
