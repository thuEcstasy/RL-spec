...
def jacobi_forward_greedy(
    ...
    if prefill_phase: # prefill phase, just compute the keys & values of prompt, return first_correct_token
        ...
        # Multi-noisy capture targets: capture_lens (list) takes precedence over capture_len (scalar).
        # `noisy_block_records[i]` is filled when accepted tokens first cross `noisy_targets[i]`.
        if capture_noisy_block:
            if capture_lens is not None:
                noisy_targets = [int(x) for x in capture_lens]
            elif capture_len is not None:
                noisy_targets = [int(capture_len)]
            else:
                noisy_targets = []
        else:
            noisy_targets = []
        noisy_block_records: List[Optional[Dict]] = [None] * len(noisy_targets)
        if fixed_window:
        ...
        while total_accepted < n_token_seq_len:
            ...
            # Capture noisy snapshots: for every target in `noisy_targets` that was just crossed
            # by the accepted-prefix length on this iteration, snapshot the current draft.
            if capture_noisy_block and noisy_targets:
                for _idx, _target in enumerate(noisy_targets):
                    if noisy_block_records[_idx] is not None:
                        continue
                    if prev_total_accepted < _target <= total_accepted:
                        noisy_full = torch.cat(
                            [accepted_n_gram[:, :prev_total_accepted].clone(), out.clone()],
                            dim=-1,
                        )[:, :n_token_seq_len]

                        noisy_block_records[_idx] = {
                            "noisy_block": noisy_full,
                            "accepted_prefix": accepted_n_gram[:, :_target].clone(),
                            "accepted_len": int(_target),
                            "itr": int(itr),
                        }
            ...
            if eos_enabled and (out[0, :num_accepted] == eos_id).any():
                ...
                return past_key_values, torch.full((1,1), eos_id, device=device, dtype=out.dtype), accepted_n_gram[:, :total_accepted], itr, noisy_block_records
            ...
            if has_rejected:
                ...
                if eos_enabled and next_token.item() == eos_id:
                    ...
                    if to_delete > 0:
                        past_key_values.delete_false_key_value(to_delete)
                    _flush_profile_pending(_current_clean_prefix_ids())
                    return past_key_values, next_token, accepted_n_gram[:, :total_accepted], itr, noisy_block_records
