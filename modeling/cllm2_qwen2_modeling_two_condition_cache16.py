...
def diffusion_forward(
    ...
    if prefill_phase: # prefill phase, just compute the keys & values of prompt
        ...
        q_logits_all = torch.full((batch, n_token_seq_len, len(tokenizer)), float('-inf'), device=input_ids.device)
        for i in range(batch):
            ...
            q_logits_all[i].scatter_(1, q_sampled[i].unsqueeze(1), 0.0)
        ...
        while True:
            ...
            for n, idx in enumerate(idx_unfin):
                ...
                if itr == 0:
                    q_scores = logits_processors(out[idx].unsqueeze(0), q_logits_all[idx]).unsqueeze(dim=0)
                else:
                    q_scores = logits_processors(out[idx].unsqueeze(0), q_logits_ids[idx.item()]).unsqueeze(dim=0)
                ...
                if itr == 0:
                    ...
                    q = q_prob.gather(-1, q_sampled[idx].unsqueeze(dim=0).unsqueeze(dim=-1))
                else:
                    p = p.gather(-1, q_sampled_ids.get(idx.item()).unsqueeze(dim=0).unsqueeze(dim=-1))
                    q = q_prob.gather(-1, q_sampled_ids.get(idx.item()).unsqueeze(dim=0).unsqueeze(dim=-1))
                    
                p = rearrange(p, '1 n 1 -> 1 n')
                q = rearrange(q, '1 n 1 -> 1 n')
                ...
                if has_rejected:
                    ...
                    if sample_additional_token:
                        q_logits_ids[idx.item()] = p_logits[num_accepted+1:-1, :]
                        q_probs = p_prob[:, num_accepted+1:-1, :]
                    else:
                        q_logits_ids[idx.item()] = p_logits[num_accepted:-1, :]
        ...
        return out, past_key_values, itr
