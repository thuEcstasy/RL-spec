...
def _get_flex_draft_blockmask(self, q_len: int, kv_len: int, draft_len: int, device):
    """
    Build (or fetch from cache) a FlexAttention BlockMask for Jacobi draft decoding.

    Semantics (absolute positions in the full sequence, i.e. including past KV):
      - causal everywhere: query at abs position q_abs can see keys k_abs <= q_abs
      - plus: the last `draft_len` positions form a draft block; every draft query
        can see every draft key (bidirectional within the block)

    Cached on the model by (q_len, kv_len, draft_len) to avoid Dynamo recompilation.
    """
    if not hasattr(self, "_jacobi_flex_mask_cache"):
        self._jacobi_flex_mask_cache = {}
    key = (int(q_len), int(kv_len), int(draft_len))
    cached = self._jacobi_flex_mask_cache.get(key)
    if cached is not None:
        return cached

    past_len = kv_len - q_len
    draft_start = kv_len - draft_len

    def mask_mod(b, h, q, k):
        q_abs = q + past_len
        causal = k <= q_abs
        in_draft = (q_abs >= draft_start) & (k >= draft_start)
        return causal | in_draft

    block_mask = create_block_mask(
        mask_mod, B=1, H=None, Q_LEN=q_len, KV_LEN=kv_len, device=device, _compile=True,
    )
    self._jacobi_flex_mask_cache[key] = block_mask
    return block_mask
...
def jacobi_forward_greedy(
    ...
    if prefill_phase: # prefill phase, just compute the keys & values of prompt, return first_correct_token
        ...
        # v4: draft block (last n_token_seq_len tokens of the prefill input) is
        # bidirectional within itself; everything else is causal.
        prefill_q_len = inputs_embeds.shape[1]
        prefill_kv_len = past_seen_tokens + prefill_q_len
        prefill_draft_len = int(n_token_seq_len) if n_token_seq_len and n_token_seq_len > 0 else 0
        prefill_mask = _get_flex_draft_blockmask(
            self,
            q_len=prefill_q_len,
            kv_len=prefill_kv_len,
            draft_len=prefill_draft_len,
        ...
        if self.model.has_sliding_layers:
            causal_mask_mapping["sliding_attention"] = prefill_mask
        ...
        noisy_block_record = None
        noisy_target = capture_len if capture_noisy_block else None
        if fixed_window:
        ...
        while total_accepted < n_token_seq_len:
            ...
            # v4: the entire current draft (`out`) is the noisy block — attends
            # bidirectionally within itself while staying causal against the
            # KV-cached prompt/prefix.
            gen_q_len = out.shape[1]
            gen_kv_len = past_seen_tokens + gen_q_len
            gen_mask = _get_flex_draft_blockmask(
                self,
                q_len=gen_q_len,
                kv_len=gen_kv_len,
                draft_len=gen_q_len,
            ...
            causal_mask_mapping = {
                "full_attention": gen_mask,
            }
            if self.model.has_sliding_layers:
                causal_mask_mapping["sliding_attention"] = gen_mask
            ...
            if eos_enabled and (out[0, :num_accepted] == eos_id).any():
                ...
                return past_key_values, torch.full((1,1), eos_id, device=device, dtype=out.dtype), accepted_n_gram[:, :total_accepted], itr, noisy_block_record
            ...
            if has_rejected:
                ...
                if eos_enabled and next_token.item() == eos_id:
                    ...
                    if to_delete > 0:
                        past_key_values.delete_false_key_value(to_delete)
                    _flush_profile_pending(_current_clean_prefix_ids())
                    return past_key_values, next_token, accepted_n_gram[:, :total_accepted], itr, noisy_block_record
