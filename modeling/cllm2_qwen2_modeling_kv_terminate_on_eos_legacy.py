...
def delete_false_key_value(self, num_of_false_tokens) -> Tuple[torch.Tensor, torch.Tensor]:
    self.key_cache, self.value_cache = zip(*[
        (k[..., :-num_of_false_tokens, :], v[..., :-num_of_false_tokens, :])
        for k, v in zip(self.key_cache, self.value_cache)
    ])
    self.key_cache, self.value_cache = list(self.key_cache), list(self.value_cache)
...
def jacobi_forward_greedy(
    ...
    if prefill_phase: # prefill phase, just compute the keys & values of prompt
        ...
        while total_accepted < n_token_seq_len:
            ...
            ###
            # first_false_index = torch.where(torch.eq(out[0, 1:], greedy_tokens[0, :]) == False)[0]
            # if first_false_index.shape[0] > 0:
            #     accepted = first_false_index[0] + 1
            # else:
            #     accepted = 1
            ###
            L = out.shape[1]
            num_accepted_raw = int(accepted)
        ...
        # NOTE: subtract 1, assuming first forward pass's drafting can be done in prefill
        return past_key_values, next_token, accepted_n_gram[:, :total_accepted], itr
...
def get_jacobi_forward_trajectory_greedy(
    ...
    if prefill_phase: # prefill phase, just compute the keys & values of prompt
        ...
        answer_trajectory_ids.append(out)
        
        # We accept the first token of input_ids from the prefilling phase
        total_accepted = 0
        itr = 0
